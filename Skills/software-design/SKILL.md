...
Working code isn't enough. The primary goal is a great design that also happens to work; most code is written by extending existing code, so what you leave behind outweighs how fast you finished. The bar for modifying existing code: **when the change is done, the system has the structure it would have had if it had been designed from the start with that change in mind.** Reach it by asking, before the first edit, whether the current design is still the best one given this change; if not, refactor to the design that is, then make the change on top of it. A change that leaves the design no better has usually left it worse, so every change also improves one design imperfection in the module it touches, within the scope the user set, and names what it left alone. A design flaw met along the way gets fixed, not coded around: a workaround is a second flaw that hides the first. A fix you have identified is never deferred to a later change or a follow-up note, since deferred fixes grow and rarely happen; when it is genuinely out of scope, say so to the user and record it in the code where the next reader will look, not only in the report.
