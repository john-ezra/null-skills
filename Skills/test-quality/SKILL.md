...
Done when every surviving candidate carries its three-part rationale.
...
Then prune: private helpers, structural wiring, incidental defaults, and construction that echoes its own arguments, unless the thing is itself an external commitment; anything whose only argument is an unexecuted line in a coverage report (meet an enforced threshold, still treat it as no evidence for any particular test); anything an existing test of equal strength already protects.

Done when deleting any retained case would leave a distinct risk unprotected and no two retained cases guard the same one.
